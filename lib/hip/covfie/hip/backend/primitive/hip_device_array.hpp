...
#include <covfie/hip/utility/memory.hpp>
...
struct hip_device_array {
    using this_t = hip_device_array<_output_vector_t, _index_t>;
    ...
    struct owning_data_t {
        ...
        {
            m_size = o.m_size;
            m_ptr = utility::hip::device_copy_d2d(
        ...
        explicit owning_data_t(
            ...
            : m_size(size)
            , m_ptr(utility::hip::device_copy_h2d(ptr.get(), size, stream))
            , m_stream(stream)
        {
        }
        ...
        explicit owning_data_t(
            ...
            : owning_data_t(
                  size, std::move(ptr), std::optional<hipStream_t>(stream)
              )
        {
        }
        ...
        std::size_t m_size;
        utility::hip::unique_device_ptr<vector_t[]> m_ptr;
        std::optional<hipStream_t> m_stream;
    };
