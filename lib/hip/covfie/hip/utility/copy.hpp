...
requires(concepts::field_backend<T> &&
    ...
    copy_backend_with_stream(U && backend, const hipStream_t & stream)
{
    constexpr bool can_construct_with_stream = std::
        constructible_from<typename T::owning_data_t, U, const hipStream_t &>;
...
requires(concepts::field_backend<typename T::backend_t> &&
    ...
    copy_field_with_stream(U && field, const hipStream_t & stream)
{
    return T(
